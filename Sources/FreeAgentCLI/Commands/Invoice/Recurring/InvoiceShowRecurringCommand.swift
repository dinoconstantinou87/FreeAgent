...
struct InvoiceShowRecurringCommand: ShowCommand {
    ...
    static let tables = [
        FieldTable<Components.Schemas.RecurringInvoice>("Items", items: { invoice in
            invoice.recurringInvoiceItems?.map { (item: $0, currency: invoice.currency) }
        }) {
            InvoiceShowCommand.itemColumns
        }
    ]

    static var sections: [FieldSection<Components.Schemas.RecurringInvoice>] {
