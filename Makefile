...
SWIFT_OPENAPI_GENERATOR_GIT_TAG ?= 1.13.0
...
API_VERSION_OUTPUT ?= $(OUTPUT_DIRECTORY)/APIVersion.swift
...
generate: bundle $(SWIFT_OPENAPI_GENERATOR_BIN) $(OPENAPI_GENERATOR_CONFIG_PATH) $(OUTPUT_DIRECTORY)  # Bundle and generate Swift sources from OpenAPI spec.
	$(SWIFT_OPENAPI_GENERATOR_BIN) generate \
		...
		"$(OPENAPI_YAML_PATH)"
	@echo "$(API_VERSION)" | grep -Eq '^[0-9]{4}-[0-9]{2}-[0-9]{2}$$' \
		|| (echo "error: info.version in $(OPENAPI_SOURCE_ROOT) must be a FreeAgent API version date (YYYY-MM-DD), found: '$(API_VERSION)'" >&2; exit 1)
	printf '// Generated by make generate, do not modify.\npublic let apiVersion = "%s"\n' "$(API_VERSION)" > "$(API_VERSION_OUTPUT)"
...
dump:  # Dump all derived values used by the Makefile.
	...
	@echo "SWIFT_OPENAPI_GENERATOR_CLONE_STAMP = $(SWIFT_OPENAPI_GENERATOR_CLONE_STAMP)"
	@echo "OPENAPI_YAML_PATH = $(OPENAPI_YAML_PATH)"
	@echo "OPENAPI_GENERATOR_CONFIG_PATH = $(OPENAPI_GENERATOR_CONFIG_PATH)"
	@echo "OUTPUT_DIRECTORY = $(OUTPUT_DIRECTORY)"
	@echo "API_VERSION_OUTPUT = $(API_VERSION_OUTPUT)"
	@echo "API_VERSION = $(API_VERSION)"
	@echo "OPENAPI_SOURCE_DIR = $(OPENAPI_SOURCE_DIR)"
	@echo "OPENAPI_SOURCE_ROOT = $(OPENAPI_SOURCE_ROOT)"

$(SWIFT_OPENAPI_GENERATOR_CLONE_STAMP):
	rm -rf "$(SWIFT_OPENAPI_GENERATOR_CLONE_DIR)" "$(OUTPUT_DIRECTORY)"
	git \
...
$(SWIFT_OPENAPI_GENERATOR_BIN): $(SWIFT_OPENAPI_GENERATOR_CLONE_STAMP)
	swift \
		build \
